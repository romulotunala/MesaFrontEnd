      ...
      <form @submit="signIn">
        ...
        <label for="password">
          ...
          <input
            type="password"
            v-model="repassword"
            :style="submitted && isError.repassword && inputError"
          />
          <div class="containerErrorMessage">
            <p class="errorMessage" v-if="submitted && isError.repassword">
              {{ isError.repassword }}
            </p>
          </div>
        </label>
        ...
        <button>{{ isFetching ? 'Registrando...' : 'Registrar'}}</button>
        <p class="option">Já possui uma conta? <a href="#" @click="OPEN_LOGIN">Entre.</a></p>
      </form>
    </div>
  </div>
</template>
...
export default {
  ...
  watch: {
    ...
    repassword(val) {
      let isError = false;
      if(!val) isError = 'Digite novamente a senha.';
      if(val && val !== this.password) isError = 'As senhas são diferentes.';
      this.isError = {
        ...this.isError,
        repassword: isError
      }
  ...
  methods: {
    ...mapMutations(['LOGIN', 'OPEN_LOGIN', 'OPEN_REGISTER', 'CLOSE_REGISTER']),
    signIn(e) {
      e.preventDefault();
      this.submitted = true;
      console.log('entrei');
